...
use TheSoftNet\LaravelAcl\Authentication\Services\ReminderService as Reminder;
use Mockery as m;
...
class ReminderServiceTest extends DbTestCase {
    ...
    /**
     * @test
     * @expectedException \TheSoftNet\LaravelAcl\Library\Exceptions\MailException
     */
    ...
    {
        $mock_mail = m::mock('TheSoftNet\LaravelAcl\Library\Email\MailerInterface')->shouldReceive('sendTo')
            ->once()
    ...
    {
        $users = $this->make('TheSoftNet\LaravelAcl\Authentication\Models\User');
        $user = $users[0];
