...
class UserControllerTest extends DbTestCase
{
    ...
    public function it_show_view_with_error_if_token_is_invalid()
    {
        ...
        $token = "_token";
        $mock_service = m::mock('StdClass')->shouldReceive('checkUserActivationCode')->once()->with($email,
            $token)->andThrow(new \TheSoftNet\LaravelAcl\Authentication\Exceptions\TokenMismatchException)->shouldReceive('getErrors')->once()->andReturn("")->getMock();
        App::instance('register_service', $mock_service);
    ...
    public function it_show_view_errors_if_user_is_not_found()
    {
        ...
        $token = "_token";
        $mock_service = m::mock('StdClass')->shouldReceive('checkUserActivationCode')->once()->with($email,
            $token)->andThrow(new \TheSoftNet\LaravelAcl\Authentication\Exceptions\UserNotFoundException())->shouldReceive('getErrors')->once()->andReturn("")->getMock();
        App::instance('register_service', $mock_service);
    ...
    public function check_for_permissions_when_adding_custom_field_type()
    {
        ...
        $fake_user = $this->make('TheSoftNet\LaravelAcl\Authentication\Models\User')[0];
        // login another user
        $this->loginAnUser();
    ...
    public function canRemoveAPermission()
    {
        ...
        $user_created = $this->make('TheSoftNet\LaravelAcl\Authentication\Models\User', array_merge($this->getUserStub(),["permissions" => [$permission_name => 1]] ) )[0];
    ...
    {
        $created_user = $this->make('TheSoftNet\LaravelAcl\Authentication\Models\User', $this->getUserStub())->first();
        $created_user_profile = $this->make('TheSoftNet\LaravelAcl\Authentication\Models\UserProfile', $this->getUserProfileStub($created_user))->first();
    ...
    {
        $mock_logged_user = m::mock('TheSoftNet\LaravelAcl\Authentication\Interfaces\AuthenticateInterface')->shouldReceive('getLoggedUser')
                             ->once()
                             ->andReturn($created_user)
                             ->getMock();
        App::instance('TheSoftNet\LaravelAcl\Authentication\Interfaces\AuthenticateInterface', $mock_logged_user);
    }
}
