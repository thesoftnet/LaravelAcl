...
class SentryAuthenticatorTest extends DbTestCase
{
    ...
    /**
     * @test
     * @expectedException \TheSoftNet\LaravelAcl\Authentication\Exceptions\AuthenticationErrorException
     **/
    ...
    /**
     * @test
     * @expectedException \TheSoftNet\LaravelAcl\Authentication\Exceptions\UserNotFoundException
     **/
    ...
    public function itCanGetActivationToken()
    {
        $mock_user = m::mock('StdClass')
                      ...
                      ->andReturn(true)
                      ->getMock();
        $mock_auth = m::mock('TheSoftNet\LaravelAcl\Authentication\Classes\SentryAuthenticator')
                      ->makePartial()
    ...
    {
        $authenticator = m::mock('TheSoftNet\LaravelAcl\Authentication\Classes\SentryAuthenticator')
                          ->makePartial()
                          ->shouldReceive('getUser')
                          ->once()
                          ->with($email)
    ...
    {
        $unbanned_user = $this->make('TheSoftNet\LaravelAcl\Authentication\Models\User', array_merge($this->getUserStub(), ["banned" => 0]))->first();
