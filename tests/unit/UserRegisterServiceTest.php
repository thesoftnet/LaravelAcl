...
class UserRegisterServiceTest extends DbTestCase
{
  ...
  {
    $mock_validator = m::mock('TheSoftNet\LaravelAcl\Authentication\Validators\UserSignupValidator')
  ...
  {
    return m::mock('TheSoftNet\LaravelAcl\Authentication\Validators\UserSignupValidator')
      ->shouldReceive('validate')
      ->once()
      ->andReturn(false)
      ->getMock();
  ...
  /**
   * @test
   * @expectedException \TheSoftNet\LaravelAcl\Authentication\Exceptions\UserExistsException
   **/
  ...
  {
    return m::mock('TheSoftNet\LaravelAcl\Authentication\Interfaces\AuthenticateInterface')
  ...
  public function setupActiveStateOfUser()
  {
    $this->disableEmailConfirmation();
    Config::set('acl_base.email_confirmation',false);
    $service = m::mock('TheSoftNet\LaravelAcl\Authentication\Services\UserRegisterService');
  ...
  public function checksForActivationAndThrowsUserNotFoundException()
  {
    ...
    {
      $service->checkUserActivationCode($email, $token);
    } catch(\TheSoftNet\LaravelAcl\Authentication\Exceptions\UserNotFoundException $e)
    {
      $gotcha = true;
    }
  ...
  {
    $user_stub =
      m::mock('TheSoftNet\LaravelAcl\Authentication\Modles\User')->makePartial()->shouldReceive('checkResetPasswordCode')->andReturn(false)->getMock();
    ...
    {
      $service->checkUserActivationCode($email, $token);
    } catch(\TheSoftNet\LaravelAcl\Authentication\Exceptions\TokenMismatchException $e)
    {
      $gotcha = true;
    }
  ...
  {
    $user_stub = new \TheSoftNet\LaravelAcl\Authentication\Models\User;
    $user_stub->activation_code = "12345_";
