...
class AuthControllerTest extends DbTestCase {
    ...
    public function setUp()
    {
        ...
        $this->current_user = $this->make('TheSoftNet\LaravelAcl\Authentication\Models\User', $this->getUserStub())->first();
    ...
    {
        $mock_reminder_service = m::mock('TheSoftNet\LaravelAcl\Authentication\Services\ReminderService')
                                  ->shouldReceive('send')
                                  ->once()
                                  ->andThrow(new AuthenticationErrorException)
                                  ->shouldReceive('getErrors')
                                  ->getMock();
        $this->app->instance('TheSoftNet\LaravelAcl\Authentication\Services\ReminderService', $mock_reminder_service);
